...
PROJECT_PATH = Path(os.environ.get("AGENT_APP_ROOT", Path(__file__).resolve().parent.parent))
MEMORY_PATH = Path(os.environ.get("AGENT_APP_MEMORY_PATH", PROJECT_PATH / "CLAUDE.md"))
...
def import_claude_memory_payloads(
    payloads: list[Any],
    ...
    for item in payloads:
        for payload in iter_payload_items(item):
            conversations_memory = _trim_text(payload.get("conversations_memory"), 80_000)
            if conversations_memory:
                for content in _markdown_memory_entries(conversations_memory):
                    found += 1
                    normalized = _normalize_memory_text(content)
                    external_id = "claude_export:memories.json:conversation:" + hashlib.sha1(
            ...
            if isinstance(project_memories, dict):
                for project_id, text in project_memories.items():
                    content = _trim_text(text, 20_000)
                    if content:
                        preference_sections.append(
                            f"Claude project memory ({project_id}):\n{content}"
                        )

    if preference_sections:
        preferences = profile.get("preferences") or {"enabled": True, "content": ""}
        current = _trim_text(preferences.get("content"), 50_000)
        marker = "Imported Claude project memories:"
        new_sections = [section for section in preference_sections if section not in current]
        if new_sections:
            addition = marker + "\n\n" + "\n\n---\n\n".join(new_sections)
            ...
            profile["preferences"] = {
                "enabled": bool(preferences.get("enabled", True)),
                "content": _trim_text(combined, 50_000),
            }
            preference_updates = len(new_sections)

    if imported or found or preference_updates:
        profile["claudeExportImport"] = {
            "checkedAt": now,
            "importedAt": now,
            "importedCount": (profile.get("claudeExportImport") or {}).get("importedCount", 0) + imported,
            "foundCount": (profile.get("claudeExportImport") or {}).get("foundCount", 0) + found,
        }
        profile["updatedAt"] = now
        _write_profile_file(profile)
    return profile, imported, found, preference_updates
